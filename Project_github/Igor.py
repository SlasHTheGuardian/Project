...
def Finish():
    ...
    DeathL1 = tkinter.Label(text="Этот день настал." +
                            Name.get() + " умер, прожив целых")
    ...
    # command= Тут написать выход из программы
    GGWP = tkinter.Button(text='Покойся с миром, любимый ' + Name.get())
...
def DayRandom():
    ...
    if D == 60:
        ...
        console = tkinter.Label(
            text='Похоже, ' + Name.get() + ' начинает различать травы.')
        console.place(x=350, y=750)
...
def HReserch():
    ...
    if lvlHRes > 2 and lvlHRes < 5:
        if HRes > 0:
            if RndHReserch < 5:
                ...
                console = tkinter.Label(
                    text='Слишком сложно, ничего не выходит. Стоит отдохнуть и попробовать снова')
        else:
            HRes += 1
    if lvlHRes == 5:
        if HRes > 0:
    ...
    if lvlHRes == 1 and Flag == 0:
        if HRes == 1:
            ...
            console = tkinter.Label(text=Name.get(
            ) + ' начинает изучать азы ботаники, наблюдая за эффектами от растений')
        if HRes == 2:
            HResImg = Image.open('1HR2.gif')
            console = tkinter.Label(
                text='Глава 1.1: Листья грецкого ореха прекрасно повышают работу иммунной системы')
        if HRes == 3:
            HResImg = Image.open('1HR3.gif')
            console = tkinter.Label(
                text='Глава 1.2: Женьшень - прекрасный имуностимулятор')
        if HRes == 4:
            HResImg = Image.open('1HRF.gif')
            console = tkinter.Label(
                text='Изучение 1 главы завершено. Теперь ' + Name.get() + ' знает чуть больше')
            RndIm = 4
            lvlHRes = 2
            Flag = 1
            HRes = 0
    ...
    if lvlHRes == 2 and Flag == 0:
        if HRes == 1:
            ...
            console = tkinter.Label(
                text='Глава 2.1: Ятрышник пятнистый помогает восстановиться после тяжелых заболеваний')
        if HRes == 2:
            HResImg = Image.open('2HR2.gif')
            console = tkinter.Label(
                text='Глава 2.2: Родиола розовая помогает повысить физическую активность')
        if HRes == 3:
            HResImg = Image.open('2HR3.gif')
            console = tkinter.Label(
                text='Глава 2.3: Шалфей содержит необходимый для человека кремний')
        if HRes == 4:
            HResImg = Image.open('2HRF.gif')
            console = tkinter.Label(
                text='Изучение 2 главы завершено. Определенно, это успех')
            RndIm = 5
            lvlHRes = 3
            Flag = 1
            HRes = 0
    ...
    if lvlHRes == 3 and Flag == 0:
        if HRes == 1:
            ...
            console = tkinter.Label(
                text='Глава 3.1: Клевер содержит много клетчатки')
        if HRes == 2:
        ...
        if HRes == 3:
            ...
            console = tkinter.Label(
                text='Глава 3.3: Зверобой - "Лекарство от 100 болезней"')
        if HRes == 4:
            HResImg = Image.open('3HRF.gif')
            console = tkinter.Label(
                text='Изучение 3 главы завершено. Эти знания точно пригодятся!')
            RndIm = 6
            lvlHRes = 4
            Flag = 1
            HRes = 0
    ...
    if lvlHRes == 4 and Flag == 0:
        if HRes == 1:
            ...
            console = tkinter.Label(
                text='Глава 4.1: Корень имбиря полезный и вкусный')
        if HRes == 2:
            HResImg = Image.open('4HR2.gif')
            console = tkinter.Label(
                text='Глава 4.2: Алое отлично смягчает, питает кожу')
        if HRes == 3:
            HResImg = Image.open('4HR3.gif')
            console = tkinter.Label(
                text='Глава 4.3: Каланхоэ перистое содержит органические кислоты')
        if HRes == 4:
            HResImg = Image.open('4HRF.gif')
            console = tkinter.Label(
                text='Изучение 4 главы завершено. Но это еще не все')
            RndIm = 8
            lvlHRes = 5
            Flag = 1
            HRes = 0
    ...
    if lvlHRes == 5 and Flag == 0:
        if HRes == 1:
            ...
            console = tkinter.Label(
                text='Глава 5.0: Экстраординарные травы: Очень сложно!')
        if HRes == 2:
            HResImg = Image.open('5HR2.gif')
            console = tkinter.Label(
                text='Глава 5.1: Элеутерококк колючий:тонизирует, избавляет от хронической усталости')
        if HRes == 3:
            HResImg = Image.open('5HR3.gif')
            console = tkinter.Label(
                text='Глава 5.2: Гинкго Билоба: улучшает концентрацию внимания, ясность мышления и память')
        if HRes == 4:
            HResImg = Image.open('5HR4.gif')
            console = tkinter.Label(
                text='Глава 5.3: Люцерна: останавливает назальные кровотечения')
        if HRes == 5:
            HResImg = Image.open('5HR5.gif')
            console = tkinter.Label(
                text='Глава 5.4: Левзея: улучшает состав крови')
        if HRes == 6:
            HResImg = Image.open('5HRF.gif')
            console = tkinter.Label(
                text='Ботаника изучена! ' + Name.get() + ' -эксперт по травам.')
            RndIm = 11
            lvlHRes = 6
            Flag = 1
            HRes = 0
    ...
    if lvlHRes == 6 and Flag == 0:
        console = tkinter.Label(text='Похоже, что ' +
                                Name.get() + ' изучил ботанику идеально')
        HResImg = Image.open('5HRF.gif')
        HRes1 = ImageTk.PhotoImage(HResImg)
        HResL = tkinter.Label(window, image=HRes1)
        HResL.place(x=450, y=500)
...
def Pravila():
    ...
    Start = tkinter.Button(
        text='Я все понял,я готов попробовать', command=PravToGame)
